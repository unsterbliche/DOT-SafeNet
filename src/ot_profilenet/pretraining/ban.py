...
class BANLayer(nn.Module):
    ...
    def forward(self, v, q, softmax=False, mask=None):
        v_num = v.size(1)
        q_num = q.size(1)
        ...
        if mask is not None:
            ...
            mask_now = mask_.expand(-1, self.h_out, -1, -1)
            att_maps = att_maps * mask_now
...
class FCNet(nn.Module):
    ...
    def __init__(self, dims, act='ReLU', dropout=0):
        ...
        layers = []
        for i in range(len(dims) - 2): #这里i只能取0吧
            in_dim = dims[i] #输入维度
            out_dim = dims[i + 1] #输出维度
            if 0 < dropout:
