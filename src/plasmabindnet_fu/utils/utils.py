...
#计算训练数据集中分子（mol）、簇（clique）和蛋白质（prot）的度分布。度分布是一个描述图中节点连接强度的统计量，通常用度直方图表示。
def compute_pna_degrees(train_loader):
    ...
    return mol_deg, clique_deg #分子和簇的度分布，mol_deg[2]表示分子中度为2的节点数量
...
def store_result(df, attention_dict, interaction_keys,  ligand_dict, 
    ...
    for idx, key in enumerate(interaction_keys):
        matching_row =  (df['smiles'] == key) # key是分子的smiles啦，取出匹配的行id，放入预测数据
        if reg_pred is not None:
            if 'predicted_activity' in df.columns:
                df.loc[matching_row, 'predicted_activity'] = reg_pred[idx]
            else:
        ...
        #         successful_ligand = store_ligand_score(key, ligand_dict[key]['atom_types'].split('|'), 
        ...
        #                 unbatched_atom_score[idx].detach().cpu().numpy()
        #                 )
    return df
...
def virtual_screening(screen_df, model, data_loader, result_path, save_interpret=True, ligand_dict=None, device='cpu',
    ...
    with torch.no_grad():
        for data in tqdm(data_loader):
            data = data.to(device)
            reg_pred_pK, reg_pred_pAC50, cls_pred, mcls_pred, sp_loss, o_loss, cl_loss, attention_dict = model(
                    # Molecule
            ...
            if reg_pred_pK is not None and reg_pred_pAC50 is not None:
                reg_pred_pK = reg_pred_pK.squeeze().reshape(-1).cpu().numpy()
                reg_preds_pK.append(reg_pred_pK)
                reg_pred_pAC50 = reg_pred_pAC50.squeeze().reshape(-1).cpu().numpy()
                reg_preds_pAC50.append(reg_pred_pAC50)
            ...
            screen_df = store_result(screen_df, attention_dict, interaction_keys, ligand_dict, 
                                     reg_pred_pK, reg_pred_pAC50, cls_pred, mcls_pred, 
                                     result_path=result_path, save_interpret = save_interpret)

    return screen_df


def eval_test(screen_df, model, data_loader, result_path, save_interpret=True, ligand_dict=None, device='cpu',
                        save_cluster=False):
    if "ID" in screen_df.columns:
        # Iterate through the DataFrame check any empty pairs 
        for i, row in screen_df.iterrows():
    ...
    with torch.no_grad():
        for data in tqdm(data_loader,file=None): #file=None是新加的
            data = data.to(device)
            reg_pred, cls_pred, mcls_pred, attention_dict = model(
                    # Molecule
                    mol_x=data.mol_x, mol_x_feat=data.mol_x_feat, bond_x=data.mol_edge_attr,
                    atom_edge_index=data.mol_edge_index, clique_x=data.clique_x, 
                    clique_edge_index=data.clique_edge_index, atom2clique_index=data.atom2clique_index,
            ...
            interaction_keys = data.mol_key # data.mol_key对应的是分子的smiles啦
            ...
            if reg_pred is not None:
                ...
                reg_truth = data.reg_y.squeeze().reshape(-1).cpu().numpy()
                reg_truths.append(reg_truth)
    ...
    if len(reg_truths) > 0: #对于某个batch来说，岂不是长度都大于0
            ...
            reg_truths = np.concatenate(reg_truths)
            # reg_preds_pK = torch.cat(reg_preds_pK).detach().cpu().numpy()
            # reg_truths_pK = torch.cat(reg_truths_pK).detach().cpu().numpy()
            # reg_preds_pAC50 = torch.cat(reg_preds_pAC50).detach().cpu().numpy()
            # reg_truths_pAC50 = torch.cat(reg_truths_pAC50).detach().cpu().numpy()
            eval_reg_result = evaluate_reg(reg_truths, reg_preds)

    return screen_df,eval_reg_result, attention_dict


def eval_test_dose(screen_df, model, data_loader, result_path, save_interpret=True, ligand_dict=None, device='cpu',
                        save_cluster=False):
    if "ID" in screen_df.columns:
        # Iterate through the DataFrame check any empty pairs 
        for i, row in screen_df.iterrows():
    ...
    with torch.no_grad():
        for data in tqdm(data_loader,file=None): #file=None是新加的
            ...
            if reg_pred is not None:
                # reg_pred_pK = reg_pred_pK.squeeze().reshape(-1).cpu().numpy()
                reg_pred = reg_pred.squeeze().reshape(-1).cpu().numpy()
                # print("预测结果1",reg_pred)
                reg_preds1.append(reg_pred)
                # 转换为列表
                reg_pred = reg_pred.tolist()
                reg_preds2.extend(reg_pred)
                # print("预测结果2",reg_pred)
           
                reg_truth = data.reg_y.squeeze().reshape(-1).cpu().numpy()
                reg_truths1.append(reg_truth)
                reg_truth = reg_truth.tolist()
                reg_truths2.extend(reg_truth)
            
    
            # screen_df = store_result(screen_df, attention_dict, interaction_keys, ligand_dict, 
            #                          reg_pred, cls_pred, mcls_pred, 
            #                          result_path=result_path, save_interpret = save_interpret,dose_mode=False)
    # print("预测结果1",reg_preds1)
    # print("预测结果2",reg_preds2)
    screen_df.loc[:, "predicted_activity"] = reg_preds2
    screen_df.loc[:, "true_activity"] = reg_truths2
    if len(reg_truths1) > 0: #对于某个batch来说，岂不是长度都大于0
            reg_preds = np.concatenate(reg_preds1)
            reg_truths = np.concatenate(reg_truths1)
            # reg_preds_pK = torch.cat(reg_preds_pK).detach().cpu().numpy()
            # reg_truths_pK = torch.cat(reg_truths_pK).detach().cpu().numpy()
            # reg_preds_pAC50 = torch.cat(reg_preds_pAC50).detach().cpu().numpy()
            # reg_truths_pAC50 = torch.cat(reg_truths_pAC50).detach().cpu().numpy()
...
def eval_predict_ppb_cmax(screen_df, model_ppb, model_cmax, data_loader, data_loader_dose, device='cpu',save_cluster=False):
    if "ID" in screen_df.columns:
        # Iterate through the DataFrame check any empty pairs 
        for i, row in screen_df.iterrows():
            if pd.isna(row['ID']):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader,file=None): #file=None是新加的
            data = data.to(device)
            reg_pred, cls_pred, mcls_pred, attention_dict = model_ppb(
                    # Molecule
                    mol_x=data.mol_x, mol_x_feat=data.mol_x_feat, bond_x=data.mol_edge_attr,
                    atom_edge_index=data.mol_edge_index, clique_x=data.clique_x, 
                    clique_edge_index=data.clique_edge_index, atom2clique_index=data.atom2clique_index,
            ...
            if reg_pred is not None:
                ...
                reg_pred = reg_pred.squeeze().reshape(-1).cpu().numpy()
                reg_pred = reg_pred.tolist()
                reg_preds_ppb.extend(reg_pred)
                # print("预测结果2",reg_pred)
    # 
  
    screen_df.loc[:, "predicted_activity_ppb"] = reg_preds_ppb
    
    model_cmax.eval()
    with torch.no_grad():
        for data in tqdm(data_loader_dose,file=None): #file=None是新加的
            data = data.to(device)
            reg_pred, cls_pred, mcls_pred, attention_dict = model_cmax(
                    # Molecule
                    mol_x=data.mol_x, mol_x_feat=data.mol_x_feat, bond_x=data.mol_edge_attr,
                    atom_edge_index=data.mol_edge_index, clique_x=data.clique_x, 
                    clique_edge_index=data.clique_edge_index, atom2clique_index=data.atom2clique_index,
            ...
            if reg_pred is not None:
                ...
                reg_pred = reg_pred.squeeze().reshape(-1).cpu().numpy()
                reg_pred = reg_pred.tolist()
                reg_preds_cmax.extend(reg_pred)
                # print("预测结果2",reg_pred)
            
    screen_df.loc[:, "predicted_activity_cmax"] = reg_preds_cmax

  

    return screen_df

def eval_predict_ppb_value(screen_df, model_ppb, data_loader, device='cpu',save_cluster=False):
...
def eval_predict_ppb(screen_df, model_ppb, data_loader, device='cpu',save_cluster=False):

    molecule_atom_attention = defaultdict(dict)
    molecule_atom_attention_dict = defaultdict(dict)
    model_ppb.eval()
    with torch.no_grad():
        for data in tqdm(data_loader,file=None): #file=None是新加的
            data = data.to(device)
            ...
            smiles_batch = data.mol_key
            atom_list = data.atom2clique_index[0].tolist()
            clique_list = data.atom2clique_index[1].tolist()
            atom_clique_list = zip(atom_list, clique_list)
            atom_attention_dict_q = {}
            for atom, clique in atom_clique_list:
                if atom not in  atom_attention_dict_q:
                     atom_attention_dict_q[atom] = []
                atom_attention_dict_q[atom].append(attention_dict['motif_attention'][clique])
            
            # 遍历atom_attention_dict，找每个atom对应的分数的最大值
            atom_attention_dict = {}
            for atom, attention_score_list in  atom_attention_dict_q.items():
                atom_attention_dict[atom] = max(attention_score_list)
            
            for atom, attention_score in atom_attention_dict.items():
                batch_idx = data.mol_x_batch[atom].item()  # 获取该原子所属分子在 batch 中的索引
                molecule_key = smiles_batch[batch_idx]  # 根据 batch 索引获取分子 SMILES 或标识
                molecule_atom_attention[molecule_key][atom] = attention_score.item() #cpu().numpy()
            # atom_attention_dict中获取了每个原子的attention，根据data.mol_x_batch，按照分子存放
            # 修改每个molecule_key对应的atom_attention_dict中atom的键的数值，每个分子对应的atom编号都从0开始
            for molecule_key, atom_attention_dict in molecule_atom_attention.items():
                atom_attenttion_dict_new = {i: atom_attention_dict[key] for i, key in enumerate(atom_attention_dict)}
                molecule_atom_attention_dict[molecule_key] = atom_attenttion_dict_new
            
            
            # counts_atom = torch.bincount(data.mol_x_batch)
            # counts_clique = torch.bincount(data.clique_x_batch)
            # # 遍历counts，找到每个分子中每个原子对应的clique编码(原始编码，不是在整个batch中的编码)，放入atom_clique_dict
            # # atom_list = []
            # # cliuqe_list = []
            # # smiles_list = []
            # atom_clique_list = zip(atom_list, clique_list)
            # for value in atom_clique_list:
            #     atom_ind = value[0]
            #     clique_ind = value[1]
            #     # 查看clique_ind所属的batch_idx
            #     batch_idx_clique = data.clique_x_batch[clique_ind].item()
            #     batch_idx_atom = data.mol_x_batch[atom_ind].item()
            #     smiles = smiles_batch[batch_idx_atom]
            #     atom_ind_n = atom_ind - torch.sum(counts_atom[:batch_idx_atom])
            #     clique_ind_n = clique_ind - torch.sum(counts_clique[:batch_idx_clique])
            #     atom_ind_n = atom_ind_n.item()
            #     clique_ind_n = clique_ind_n.item()
            #     batch_idx_atom = batch_idx_atom
            #     # 如果atom_ind_n在atom_clique_dict[batch_idx_atom]中不存在，则创建一个新list，反之直接append加入
            #     if atom_ind_n not in atom_clique_dict[batch_idx_atom]:
            #         atom_clique_dict[batch_idx_atom][atom_ind_n] = [] #改list存放
            #     else:
            #         atom_clique_dict[batch_idx_atom][atom_ind_n].append(clique_ind_n)
            #     # atom_clique_dict[batch_idx_atom][atom_ind_n] = [] #改list存放
            #     # atom_clique_dict[batch_idx_atom][atom_ind_n].append(clique_ind_n)
            #     # {0: {atom_ind_n1: clique_ind_n1,atom_ind_n2:clique_ind_n1,}}
            
            # # print(len(atom_clique_dict))
            # # print(len(smiles_batch))
            # for i in range(len(smiles_batch)):
            #     atom_clique_dict[smiles_batch[i]] = atom_clique_dict.pop(i)
                
    
    return molecule_atom_attention_dict