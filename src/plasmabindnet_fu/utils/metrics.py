...
# def evaluate_reg(Y, F, Y2, F2):
#     not_nan_indices = ~np.isnan(Y)
#     Y = Y[not_nan_indices]
#     F = F[not_nan_indices]

#     not_nan_indices2 = ~np.isnan(Y2)
#     Y2 = Y2[not_nan_indices2]
#     F2 = F2[not_nan_indices2]
    
#     return { 
#         'mse': float((get_mse(Y,F) + get_mse(Y2,F2))/2),
#         'rmse': float((get_rmse(Y,F) + get_rmse(Y2,F2))/2),
#         'mae': float((get_mae(Y,F) + get_mae(Y2,F2))/2),
#         'sd': float((get_sd(Y,F) + get_sd(Y2,F2))/2),
#         'pearson': float((get_pearson(Y,F) + get_pearson(Y2,F2))/2),
#         'spearman': float((get_spearman(Y,F) + get_spearman(Y2,F2))/2),
#         'rm2': float((get_rm2(Y,F) + get_rm2(Y2,F2))/2),
#         'ci': float((get_cindex(Y,F) + get_cindex(Y2,F2))/2)
#     }
