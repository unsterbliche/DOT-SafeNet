...
def atom_features(atom):
    ...
    #编码时选择范围到 10 是为了覆盖可能的特殊情况，确保适用于更广泛的分子数据集。
    ...
    return np.array(encoding) # 原子特征维度为43
...
class MoleculeGraphDataset():
    def __init__(self,atom_classes=None, halogen_detail=False, save_path=None):
        ...
        if atom_classes is None:
            ...
            if halogen_detail:
                ...
                atom_classes = [
                    ...
                    (34, 'Se'),
                    ## halogen
                    ([9, 17, 35, 53], 'halogen'), #halogen表示卤素
                    ## halogen
                    (metals, 'metal')
                ]
        ...
        for code, (atom, name) in enumerate(atom_classes):
            if type(atom) is list:
                ...
                self.ATOM_CODES[atom] = code #self.ATOM_CODES字典长度是89，但是atom_classes是9，即89个键对应的值只有9个（0~8）
    ...
    def mol_simplified_feature(self,mol):
        ...
        for atom in mol.GetAtoms():
            atom_ids.append(atom.GetIdx())
            atomic_num = atom.GetAtomicNum() # 返回在元素周期表中的序数

            if atomic_num in self.ATOM_CODES.keys(): #self.ATOM_CODES的键是各个元素在元素周期表中的序号（包含89个元素），值是该元素的位置序号（只有9个）
                atom_feats.append([self.ATOM_CODES[atomic_num] + 1])
            else:
                atom_feats.append([0])
        feature = np.array(list(zip(*sorted(zip(atom_ids, atom_feats))))[-1]) #排序之后取atom_feats列表，即每个原子的编号（包含种类信息），相同的原子有相同的编号
    ...
    #         v2 = b.GetEndAtomIdx()
    #         b_type = self.edge_dict[b.GetBondType()] #获取边的类型
    ...
    def bond_feature_adj(self,mol):
        ...
        for bond in bonds:
            ...
            edge_indices.append([v2, v1])
        
        edge_indices = np.array(edge_indices).T #转置也可以放在后面的dataset.py文件中

        return edge_indices
    
    def bond_feature(self,mol, use_chirality=True):
        bonds = mol.GetBonds()
        bond_ids = []
        bond_feats = []
        for bond in bonds:
            ...
            if use_chirality:
                bond_feat = bond_feat + one_of_k_encoding_unk(
                    str(bond.GetStereo()),
                    ["STEREONONE", "STEREOANY", "STEREOZ", "STEREOE"])
            # bond_feats.append(bond_feat)
            bond_feats += [bond_feat]*2 # 双向边
        
        feature = np.array(bond_feats) #后面在dataset中转float就好
        # 把feature中的Ture转换为1，False转换为0
        # feature = feature.astype(float)
        # feature = np.array(list(zip(*sorted(zip(bond_ids, bond_feats))))[-1])
        # 边是双向的
        

        return feature

    def junction_tree(self,mol): #对分子进行树分解，每个节点表示一个簇，边表示簇之间的连接
        tree_edge_index, atom2clique_index, num_cliques, x_clique = tree_decomposition(mol,return_vocab=True)
        #tree_edge_index 二维数组，表示树中的边；atom2clique_index 二维数组，表示分子中的原子分配到哪个簇；num_cliques簇的数量；x_clique一维数组，表示簇的类别
        ## if weird compounds => each assign the separate cluster
        if atom2clique_index.nelement() == 0: #出现孤立节点的问题解决
    ...
    def featurize(self,mol,type='atom_type'):
        ...
        # bond_feature_adj = self.bond_feature_adj(mol)
        edge_indices = self.bond_feature_adj(mol)
        bond_feature = self.bond_feature(mol)

        return atom_feature, bond_feature, edge_indices
...
def tree_decomposition(
    ...
    cliques = [list(x) for x in Chem.GetSymmSSSR(mol)] #获取对称单环系统
    xs = [0] * len(cliques)
    ...
    atom2clique = [[] for i in range(mol.GetNumAtoms())] #是列表，每个元素是一个列表，表示原子分配到哪些簇
    for c in range(len(cliques)):
        for atom in cliques[c]:
            atom2clique[atom].append(c)

    # Merge rings that share more than 2 atoms as they form bridged compounds.共享超过两个原子的环合并为一个簇
    ...
    xs = [x for x in xs if x >= 0] #每个元素表示对应簇的类型，0表示环，1表示非环的化学键，2表示合并后的环，3表示单原子簇

    # Update `atom2clique` mappings. 更新`atom2clique`映射
...
## edge_index 二维数组，表示树中的边；atom2clique_index 二维数组，表示分子中的原子分配到哪个簇；len(cliques)簇的数量；vocab(x_clique)一维数组，表示簇的类别
...
def smiles2graph(m_str):
    ...
    #mol = get_mol(m_str)
    atom_feature, bond_feature, edge_indices = mgd.featurize(mol,'atom_full_feature')
    atom_idx, _, _ = mgd.featurize(mol,'atom_type')
    ...
    out_dict = {
        ...
        'edge_indices':torch.tensor(edge_indices),#.to(torch.int8),之前没加

    }
    tree['tree_edge_index'] = tree['tree_edge_index']#.to(torch.int8) 二维数组，表示树中的边
    tree['atom2clique_index'] = tree['atom2clique_index']#.to(torch.int8) 二维数组，表示分子中的原子分配到哪个簇
    tree['x_clique'] = tree['x_clique']#.to(torch.int8) 一维数组，表示簇的类别
...
def ligand_init(smiles_list):
    ...
    for smiles in tqdm(smiles_list):
        ligand_dict[smiles] = smiles2graph(smiles) #同一个smiles之对应一种表征
