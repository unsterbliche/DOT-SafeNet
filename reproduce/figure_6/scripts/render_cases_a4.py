...
def main():
    ...
    for ext in ["png", "pdf", "svg"]:
        legend_fig.savefig(OUT / f"figure_6_case_legend.{ext}", dpi=300, bbox_inches="tight")
    plt.close(legend_fig)
    pd.concat(target_tables, ignore_index=True).to_csv(OUT / "figure_6_case_target_source.csv", index=False)
