...
def histogram(values, xlabel, title, color, stem, cfg, bins=10):
    ...
    ax.hist(values, bins=bins, color=color, alpha=cfg["colors"]["histogram_alpha"],
...
def dose_histogram(cmax, cfg):
    ...
    save(fig, "supplementary_figure_6e_normalized_dose_intervals", cfg)
    plt.close(fig)
...
def dataset_table(ppb_counts, cmax_counts, cfg):
    ...
    save(fig, "supplementary_figure_6f_dataset_sample_counts", cfg)
    plt.close(fig)
    return table
...
def tsne_coordinates(ppb, cmax, cfg):
    ...
    tsne_args = {
        ...
        "init": "pca",
        "learning_rate": "auto",
        "random_state": cfg["tsne"]["seed"],
        "method": "barnes_hut",
    }
    iteration_parameter = "max_iter" if "max_iter" in inspect.signature(TSNE).parameters else "n_iter"
    tsne_args[iteration_parameter] = cfg["tsne"]["iterations"]
    coords = TSNE(**tsne_args).fit_transform(reduced)
...
def tsne_legend(cfg):
    ...
    fig = plt.figure(figsize=(4.8, 0.42))
    fig.legend(handles=[
        Line2D([0], [0], marker="o", linestyle="none", markerfacecolor=cfg["colors"]["chembl"],
               markeredgecolor="none", markersize=5, label="ChEMBL"),
        Line2D([0], [0], marker="o", linestyle="none", markerfacecolor=cfg["colors"]["task"],
               markeredgecolor="none", markersize=5, label="PPB / Cmax"),
    ], loc="center", ncol=2, frameon=False)
    for ext in cfg["export"]["formats"]:
        fig.savefig(legend_dir / f"supplementary_figure_6g_h_legend.{ext}",
...
def make_composite(cfg):
    names = [
        ("supplementary_figure_6a_ppb_distribution", "supplementary_figure_6b_unbound_fraction_log10_distribution"),
        ("supplementary_figure_6c_cmax_distribution", "supplementary_figure_6d_cmax_log10_distribution"),
        ("supplementary_figure_6e_normalized_dose_intervals", "supplementary_figure_6f_dataset_sample_counts"),
        ("supplementary_figure_6g_chemical_space_ppb", "supplementary_figure_6h_chemical_space_cmax"),
    ]
    width, row_h, margin, gap = 2160, 570, 34, 22
    ...
    canvas.save(OUT / "supplementary_figure_6.png", dpi=(cfg["figure"]["dpi"],)*2)
    canvas.save(OUT / "supplementary_figure_6.pdf", "PDF", resolution=cfg["figure"]["dpi"])
...
def main():
    ...
    histogram(ppb_percent, "PPB (%)", "PPB distribution", cfg["colors"]["ppb"],
              "supplementary_figure_6a_ppb_distribution", cfg, bins=np.arange(0, 110, 10))
    histogram(ppb["activity"], r"$\log_{10} f_{u}$", "Unbound fraction distribution",
              cfg["colors"]["ppb"], "supplementary_figure_6b_unbound_fraction_log10_distribution", cfg, bins=10)
    histogram(cmax_value, r"Cmax ($\mu$g mL$^{-1}$)", "Cmax distribution",
              cfg["colors"]["cmax"], "supplementary_figure_6c_cmax_distribution", cfg, bins=10)
    histogram(cmax["activity"], r"$\log_{10}$ Cmax ($\mu$g mL$^{-1}$)",
              "Log-transformed Cmax distribution", cfg["colors"]["cmax"],
              "supplementary_figure_6d_cmax_log10_distribution", cfg, bins=10)
    ...
    tsne_panel(coords, "in_ppb", "PPB", "supplementary_figure_6g_chemical_space_ppb", cfg)
    tsne_panel(coords, "in_cmax", "Cmax", "supplementary_figure_6h_chemical_space_cmax", cfg)
    tsne_legend(cfg)
