...
DATA = ROOT / "data" / "ppb_scaffold_metrics" / "supplementary_table_2_plasmabindnet_and_fu.csv"
OUT = ROOT / "outputs" / "ppb_scaffold_metrics"
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "params_ppb_scaffold_metrics.yaml")
    ...
    for metric in ("RMSE", "MAE"):
        ...
        panel = "a" if metric == "RMSE" else "b"
        save(fig, f"supplementary_figure_7{panel}_{metric.lower()}", cfg)
        plt.close(fig)
    ...
    save(fig, "supplementary_figure_7a_b", cfg)
