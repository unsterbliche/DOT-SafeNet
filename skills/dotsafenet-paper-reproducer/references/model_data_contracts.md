...
The DOT-SafeNet feature table contains `Drug`, `smiles`, `cmax_free(uM)`, 194 UniProt activity columns, and 194 `accession*Cmax` columns. The numerical model input has 389 values. `data/model/target_order.csv` defines the exact column order.

## SOC table

The label table contains SMILES followed by 18 MedDRA SOC columns. Values are 0, 1, or missing. Missing labels remain excluded from loss and evaluation. `data/model/soc_order.csv` defines the exact task order.
