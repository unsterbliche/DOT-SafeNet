...
SELECTED_SOC = {"GAS","CAR","REP","VAS","REN"}
...
order_table.to_csv(DATA / "figure5a_module_ordered_target_columns.csv", index=False)
mat.to_csv(DATA / "figure5a_module_ordered_evidence_matrix.csv")
...
for ext in ["png","pdf","svg"]:
    fig.savefig(OUT / f"figure5a_module_ordered_target_landscape.{ext}", dpi=300, bbox_inches="tight")
plt.close(fig)
...
for ext in ["png","pdf","svg"]:
    legend.savefig(OUT / f"figure5a_module_ordered_target_legend.{ext}", dpi=300, bbox_inches="tight")
plt.close(legend)